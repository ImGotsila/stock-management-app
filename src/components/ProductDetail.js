...
import { MinusCircle, PlusCircle, ArrowLeft, Package, Edit, Save, XCircle } from 'lucide-react';
  ...
  if (!product) {
    return (
      <div className="card shadow-sm text-center p-5">
        ...
        <p className="text-muted mb-4">สินค้าที่คุณกำลังมองหาอาจไม่มีอยู่ในระบบแล้ว</p>
  ...
  return (
    <div className="container py-4">
        ...
        <div className="card-body">
          <div className="row g-4">
            <div className="col-md-4">
              <div className="product-image-large bg-light rounded-3 d-flex align-items-center justify-content-center overflow-hidden" style={{ height: '400px' }}>
                <img
                  ...
                  onError={(e) => {
                    ...
                    // Check if nextSibling exists before accessing style
                    if (e.target.nextSibling) {
                        e.target.nextSibling.style.display = 'flex';
                    }
                  }}
                />
                <div className="no-image-large text-muted opacity-50 fs-1" style={{display: 'none'}}>
                  📷
                </div>
              </div>
            </div>
            <div className="col-md-8">
              <h2 className="display-5 text-dark mb-3">{product.productName}</h2>
              <p className="text-muted mb-1">รหัสสินค้า: {product.productId}</p>
              <p className="text-muted text-uppercase mb-4">หมวดหมู่: {product.category}</p>
              <div className="bg-primary text-white p-3 rounded-pill d-flex align-items-center justify-content-center gap-2 mb-4">
                <Package size={24} />
                <span className="fw-bold fs-5">สต็อกคงเหลือทั้งหมด: {product.totalStock} ชิ้น</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="card shadow-sm">
        <div className="card-body">
          <h3 className="h4 text-dark mb-4 text-center">จัดการสต็อกและราคาตามขนาด</h3>
                      ...
                      {isEditing ? (
                        <div className="price-edit-form">
                          ...
                          <div className="d-flex justify-content-center gap-2">
                            <button onClick={() => handleSavePrice(size)} className="btn btn-success btn-sm d-flex align-items-center">
                              <Save size={16} className="me-1" /> บันทึก
                            </button>
                            <button onClick={handleCancelPrice} className="btn btn-secondary btn-sm d-flex align-items-center">
                        ...
                        <>
                          <p className="mb-1">ปลีก: ฿{retailPrice.toLocaleString()}</p>
                          <p className="mb-3">ส่ง: ฿{wholesalePrice.toLocaleString()}</p>
                          <button onClick={() => handleEditPriceClick(size)} className="btn btn-outline-primary btn-sm d-flex align-items-center mx-auto">
                      ...
                      <>
                        <div className="d-flex justify-content-center gap-2 mb-3">
                          <button
                            ...
                            className="btn btn-danger rounded-circle p-2 d-flex align-items-center justify-content-center" style={{ width: '45px', height: '45px' }}
                          >
                            <MinusCircle size={20} />
                          </button>
                          <button
                            onClick={() => handleUpdateStock(size, 1)}
                            className="btn btn-success rounded-circle p-2 d-flex align-items-center justify-content-center" style={{ width: '45px', height: '45px' }}
                          >
                            <PlusCircle size={20} />
                          </button>
                        </div>
      ...
      {notification && (
        <div className={`notification alert alert-dismissible fade show ${notification.type === 'success' ? 'alert-success' : 'alert-danger'}`} role="alert">
